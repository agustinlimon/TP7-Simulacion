export module HTMLUtils {
  ...
  export function limpiarTablaSimulacion(tabla: HTMLTableElement, cantEncabezados: number, cantSubEncabezados: number) {
    ...
    // Limpiamos los encabezados correspondientes a los clientes.
  ...
  // Completa los encabezados de la tabla con los datos de los clientes.
  export function completarEncabezadosClientes(cantidad: number, tabla: HTMLTableElement, columnas: string[]): void {
    ...
    for (let i: number = 0; i < cantidad; i++) {
      ...
      col.appendChild(document.createTextNode('Cliente N° ' + (i+1)));
